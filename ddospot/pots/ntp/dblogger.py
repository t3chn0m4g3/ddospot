...
try:
    ...
    from sqlalchemy import Column, ForeignKey, Integer, SmallInteger, DateTime, LargeBinary
...
class DBThread(dbbase.DBBaseThread):
    ...
    class Attack(Base):
        __tablename__ = 'ntpot_attack'

        src_id = Column(Integer, ForeignKey('ntpot_sources.src_ip'), primary_key=True)
        mode = Column(SmallInteger, default=7, primary_key=True)
    ...
    def _update_attack(self, db_params):
        for attack in self.session.query(DBThread.Attack).\
                                   ...
                                   filter(DBThread.Attack.mode == db_params['mode']):
    ...
    def _load_requests(self):
        ...
        for attack in self.session.query(
                                        ...
                                        DBThread.Attack.mode,
                                        func.max(DBThread.Attack.latest).label('latest'),
                                        DBThread.Attack.count,
                                        ).group_by(DBThread.Attack.src_id):
            ip = attack.src_id
            request_log[ip] = {}
            request_log[ip]['mode'] = attack.mode
            request_log[ip]['last_seen'] = attack.latest
            request_log[ip]['count'] = attack.count

        return request_log

    def _get_detailed_statistics(self):
        # statistics specific for NTP honeypot
        detailed_stats = {}
        ...
        modes = []
        detailed_stats['modes'] = modes
        for attack in self.session.query(
                                        DBThread.Attack.mode.label('mode'),
                                        func.count(DBThread.Attack.mode).label('mode_count'),
                                        ).group_by('mode').order_by(desc('mode_count')).limit(5):
            modes.append((attack.mode, attack.mode_count))
